...
This file contains all study-oriented and recruiter-oriented technical notes extracted from README item 2 onward.
...
Why: an IdP/SSO product must handle multiple clients, federation, tokens, consent, key rotation, metadata, etc.
...
- Redirect HTTP traffic to HTTPS for securing encryptation on credentials (not implemented)
...
### `api_keys`
- `id` (uuid)
- `user_id` (fk) (attach the api-key to a existing user for better management)
- `prefixed_key_hash` (SHA-256 for security and performance) (should be timing-attacks safe) (we store prefix + hashed key for avoiding concat logic with prefix + keyhash)
- `prefix` (first 8 chars of the raw key, for support/logging) (separeted field for fast lookup)
- `created_at` (date) 
...
- `GET /sensitive/hello`
- `POST /api-keys/revoke` (not implemented)
...
* Repository - "seamapi/prefixed-api-key"
